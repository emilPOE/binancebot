...
while True:
    try:
        ...
        total_balance = initial_balance + (open_order_value - 83)
        ...
        print(f"Current TUSD Balance: {tusd_balance['free']}. Total Balance (including {num_open_orders} open limit sell orders): {(total_balance + 83):.4f}")
        ...
        if tusd_balance is not None and decimal.Decimal(tusd_balance["free"]) >= decimal.Decimal(tusd_amount):
            ...
            profit = 0.0005 * (1 + (num_open_orders - 2))
    ...
    num_open_orders = len(client.get_open_orders(symbol=symbol))
    wait_time = (60 * 5) * (2 ** (num_open_orders - 2))
